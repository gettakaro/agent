...
function EditAgentPage() {
  ...
  // Extract custom agent ID (remove 'custom:' prefix)
  const isCustomAgent = agentId.startsWith('custom:');
  const customAgentId = isCustomAgent ? agentId.replace('custom:', '') : undefined;

  const { data: agent, isLoading: agentLoading } = useCustomAgentQuery(customAgentId);
  ...
  const updateAgent = useUpdateCustomAgentMutation();
  ...
  useEffect(() => {
    if (agent && !initialized) {
      ...
      setSelectedKBs(agent.knowledgeBases || []);
      setInitialized(true);
    }
  }, [agent, initialized]);

  const isLoading = agentLoading || modelsLoading || toolsLoading || kbLoading;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      navigate({ to: `/agents/${encodedAgentId}` });
    } catch (err) {
  ...
  if (!isCustomAgent) {
    return (
      <PageContainer>
        ...
        <ErrorMessage>Only custom agents can be edited</ErrorMessage>
      </PageContainer>
    );
  }
  ...
  if (!agent) {
    return (
      <PageContainer>
        ...
        <ErrorMessage>Agent not found</ErrorMessage>
      </PageContainer>
    );
  }
  ...
  return (
    <PageContainer>
      <BackLink to={`/agents/${encodedAgentId}`}>
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
          <path d="M19 12H5M12 19l-7-7 7-7" />
        </svg>
        Back to Agent
      ...
      <Form onSubmit={handleSubmit}>
        ...
        <FormActions>
          ...
          <Button type="button" onClick={() => navigate({ to: `/agents/${encodedAgentId}` })}>
            Cancel
          </Button>
        </FormActions>
      </Form>
