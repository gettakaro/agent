...
export class ApiKeyService {
  async saveApiKey(userId: string, provider: string, apiKey: string): Promise<void> {
    ...
    const existing = await db("user_api_keys").where({ user_id: userId, provider }).first();

    if (existing) {
      await db("user_api_keys").where({ user_id: userId, provider }).update({
        api_key: apiKey,
        updated_at: new Date(),
      });
    } else {
  ...
  async getApiKey(userId: string, provider: string): Promise<string | null> {
    const db = getDb();
    const row = await db("user_api_keys").where({ user_id: userId, provider }).first();
  ...
  async listProviders(userId: string): Promise<string[]> {
    const db = getDb();
    const rows = await db("user_api_keys").where({ user_id: userId }).select("provider");
