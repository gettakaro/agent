...
export async function vectorSearch(
  ...
  let sqlQuery = db(TABLE)
    .select("content", "metadata", db.raw("1 - (embedding <=> ?::vector) as score", [`[${queryEmbedding.join(",")}]`]))
  ...
  if (filters) {
    for (const [key, value] of Object.entries(filters)) {
      if (value !== undefined) {
        sqlQuery = sqlQuery.whereRaw("metadata->>? = ?", [key, String(value)]);
      }
    }
  }
  ...
  const results = await sqlQuery
    .whereRaw("1 - (embedding <=> ?::vector) >= ?", [`[${queryEmbedding.join(",")}]`, minScore])
